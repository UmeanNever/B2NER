...
matplotlib.rcParams['font.sans-serif'] = ['SimHei']  # for example, 'SimHei'
...
class MetricF1NA(MetricF1):
    ...
    def update(self, y_truth: set, y_pred: set):
        ...
        for truth in y_truth:
            if ',na,' in truth or ',没有关系,' in truth:
...
class AuditBase:
    def __init__(self, record_limit=RECORD_LIMIT):
...
class AuditAnyF1NotOne(AuditBase):
    "检测任意F1 score不为1"
    def _check(self, last) -> bool:
        last_metric = last['metric']
        if hasattr(last_metric, 'last_TP'):
            if len(last['y_pred']) != 0 or len(last['y_truth']) != 0:
                return (last_metric.last_FN + last_metric.last_FP) != 0
        return False
...
class EvaluatorBase:
    ...
    def _resolve_option(s):
        "s: instruction"
        option_parts = re.findall('Label Set:(.+?)\n', s)
        zh_option_parts = re.findall('标签范围:(.+?)\n', s)
        parts = option_parts + zh_option_parts
        if len(parts) <= 0:
            return []
        part = parts[0]
        ans = [EvaluatorBase._format(x) for x in part.split(',')]
        return ans
    ...
    def _format(s):
        ...
        s = s.replace('located', 'location')
        s = s.replace('organisation', 'organization')
        # remove english and chinese book mark and quatation marks at start and end of an entity
        if ":" in s:
            other_parts = s.split(':')[:-1]
            last_part = s.split(':')[-1]
            # extract content between 《 and 》
            last_part = re.sub(r'^[@#]+', '', last_part)
            last_part = re.sub(r'[@#]+$', '', last_part)
            extracted = re.findall(r'《(.+?)》', last_part)
            last_part = extracted[0] if len(extracted) > 0 else last_part
            # extract content that starts and ends with [《》<>（）()]
            extracted = re.findall(r'^[《》<>（）()]+(.+?)[《》<>（）()]+$', last_part)
            last_part = extracted[0] if len(extracted) > 0 else last_part
            s = ':'.join(other_parts + [last_part])
        return s
...
class EvaluatorSentiment(EvaluatorBase):
    def _init_metric(self):
        self.metric = MetricAcc()
    def _extract(self, json_data, predict):
        y_truth = self._remove_redundant_space(json_data['Instance']['ground_truth'])
        y_pred = self._remove_redundant_space(predict)
        if ":" in y_pred:
...
class EvaluatorEET(EvaluatorBase):
    ...
    def _extract(self, json_data, predict: str):
        ...
        # event a: trigger; event b: trigger
        entity_truth = set()
        for ent in self._format(json_data['Instance']['ground_truth']).split(';'):
            ent = self._format(ent)
            # if ':' not in ent:
